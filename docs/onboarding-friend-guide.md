...
This guide walks you through everything you need to start using your personal AI assistant hosted on the Tollgate infrastructure. You will learn how to install the FIPS mesh software, configure a peer connection, get a Nostr key, install the Buzz chat client, connect to the relay, authenticate with your Nostr identity, talk to your Hermes AI agent, manage kanban tasks, dispatch workers, understand quality gates, use Cashu credits, SSH between mesh machines, and fix common problems.
...
If the command is not found, make sure `/usr/bin/fipsctl` exists and the package installed correctly. If the service failed to start, see Section 14.
...
After about 30 seconds you should see Andre listed as `connected`. If not, see Section 14 (Troubleshooting).
...
If you want a key before installing Buzz (for example, to send your npub to your operator first), generate one from a terminal:
...
Or using the `nak` Nostr CLI:

```bash
nak key generate
```

Save the `nsec1...` somewhere safe (a password manager is ideal). You will import it into Buzz in the next section. Send the `npub1...` to your operator.
...
## 5. Connecting to the Relay

The relay is the server that routes messages between you and your Hermes agent. It uses the Nostr protocol over WebSocket. Your operator runs one private relay:
...
This relay hosts the NIP-29 group chats where your Hermes bot lives, and it enforces NIP-42 authentication (next section), so only invited keys can connect.
...
- Check the subdomain spelling — it must be `relay.<your-domain>`, not a public relay
...
- Check that you are connected to the correct relay (Section 5).
...
Hermes processes your message, routes it through the LLM proxy to the AI model, and returns the response. Response time is typically 2-10 seconds depending on complexity.

### What Hermes can do

- **Answer questions** — general knowledge, coding help, research, writing
- **Create and manage tasks** — using the kanban system (Section 9)
- **Dispatch workers** — spin up sub-agents for parallel work (Section 10)
- **Run quality gates** — enforce testing and review standards (Section 11)
...
## 9. Kanban Boards and Task Management
...
Each task can be assigned to a worker profile. By default, Hermes works on tasks itself. For parallel work, you can dispatch tasks to specialist workers (Section 10).
...
1. You create a task on a kanban board (Section 9).
...
## 12. Cashu Credits Flow

Cashu is a Bitcoin-based ecash system that handles AI credits. Your Hermes instance uses Cashu tokens to pay for LLM inference. Credits are issued by your operator and consumed as you use the AI.
...
Operator issues credits
    ...
    → Tokens spent at the LLM proxy for each AI call
    → Balance decreases per token of AI usage
```

Behind the scenes, every AI request is routed through an LLM proxy (routstrd) that pays for the call with your Cashu tokens. You do not need to manage this — it is automatic. The operator configures the proxy and the Cashu wallet when your instance is set up.
...
Hermes queries the LLM proxy for your current token balance and reports it. Balances are denominated in satoshis (sat), the smallest unit of Bitcoin.
...
Each friend has their own API key and Cashu wallet with its own quota. Your usage does not affect other friends' credits, and theirs does not affect yours. The operator manages all quotas centrally.

---

## 13. SSH via the FIPS Mesh
...
If Hermes reports zero or an error, message your operator. They can check the LLM proxy and mint-orchestrator status.
...
| AI credits | Cashu via LLM proxy | `How many credits do I have?` |
...
2. Run the quick health check in Section 14.
