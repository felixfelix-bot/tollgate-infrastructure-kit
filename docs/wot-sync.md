...
```bash
# Blossom only
ansible-playbook ansible/playbooks/41-wot-sync.yml -l vps1 --tags blossom_wot
# grasp-mirror only
ansible-playbook ansible/playbooks/41-wot-sync.yml -l vps1 --tags grasp_wot
```
...
| `wot_sync_root_npub` | (unset — export `ROOT_NPUB` or pass `--root-npub`) | root npub for kind-3 fetch fallback |
